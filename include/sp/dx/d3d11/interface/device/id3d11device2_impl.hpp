...
{
    this->_interface->GetImmediateContext2(ppImmediateContext);
}
...
{
    HRESULT result = this->_interface->CreateDeferredContext2(ContextFlags, ppDeferredContext);
...
{
    this->_interface->GetResourceTiling(
                    pTiledResource,
...
{
    HRESULT result = this->_interface->CheckMultisampleQualityLevels1(Format, SampleCount, Flags, pNumQualityLevels);
